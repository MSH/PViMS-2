...
{
    public class PatientExpandedByCustomAttributeQueryHandler
        : IRequestHandler<PatientExpandedByCustomAttributeQuery, PatientExpandedDto>
    {
        ...
        private readonly ILogger<PatientExpandedByCustomAttributeQueryHandler> _logger;
        private readonly ICustomAttributeService _customAttributeService;

        public PatientExpandedByCustomAttributeQueryHandler(
            ...
            IMapper mapper,
            ILogger<PatientExpandedByCustomAttributeQueryHandler> logger,
        ...
        public async Task<PatientExpandedDto> Handle(PatientExpandedByCustomAttributeQuery message, CancellationToken cancellationToken)
        {
            var cohortGroupIdParm = new SqlParameter("@CohortGroupID", message.CohortGroupId.ToString());
            var customAttributeKeyParm = new SqlParameter("@CustomAttributeKey", "Medical Record Number");
            var customPathParm = new SqlParameter("@CustomPath", "CustomStringAttribute");
            var customValueParm = new SqlParameter("@CustomValue", message.MedicalRecordNumber);

            //var patientsFromRepo = _context.PatientLists
            //    .FromSqlRaw<PatientList>($"EXECUTE spSearchPatientsByCustomAttribute @CohortGroupID, @CustomAttributeKey, @CustomPath, @CustomValue"
            //        , cohortGroupIdParm
            //        , customAttributeKeyParm
            //        , customPathParm
            //        , customValueParm)
            //    .AsEnumerable();
            List<Patient> patientsFromRepo = new List<Patient>();
            ...
            var patientFromRepo = await _patientRepository.GetAsync(p => p.Id == patientsFromRepo.First().Id, new string[] {
